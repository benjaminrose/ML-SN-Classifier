...
# simlist    -list of simulated samples (Training, Test/Validation)
# datalist   -list of "data" samples (Test/Validation, Spec etc)
...
# true       -dict of masks for true types 
...
# alt type for plots multiple cuts
alt = 'alt'
altIa = alt + Ia
altCC = alt + CC
altIbc = alt + Ibc
...
def parse_args(argv):
    ...
    parser.add_argument('--data', choices=[Test, Spec, Spec_nofp, Phot], default=[Phot], nargs='+',
...
def score_func(probs, y, Fix_eff):
    Ia = (y == 0)
    CC = (y > 0)
...
def score_func_est(estimator, X, y, Fix_eff):
...
def get_purity_scores(yset, probs, Fix_eff):
    ...
    global score
    score = score_func(probs, yset, Fix_eff)
                                    ...
                                    if (SNTotals[dkey][t] > 0):
                                        ...
                                        if (t in savetypes):
                                            tee('      {:11} {:2}{:6}  {:6} {:10.3f} {:8} {:10.3f}'.format(par, op,
                                                str(mcut), np.sum(Bazincuts[filt][dkey][t][key]),
                                                Bazincuts[filt][dkey][t][key + _eff],
                                                np.sum(Bazincuts[filt][All][dkey][t]),
                                                Bazincuts[filt][ All][t + _eff]),
                                                ff)
                                    # else:
                                    #    tee('    No {} data for type {}'.format(dkey,t),ff)
                            else:
...
def main(args):
    ...
    f = fig.add_subplot(232)
    plot_types(p2types, 'c', alldata, plotlist=p2list, xlabel='SALT $c$', ylabel='Number', plotdict=plotdict, bins=cbins,
               title=title)
    ...
    plot_types(p2types, 'fit_pr', alldata, plotlist=p2list, xlabel='$f_p$', ylabel='Number', plotdict=plotdict, bins=p_bins,
               title=title)
    f.set_xlim(0, 1.0)
    f.legend(loc='best', fontsize='small', numpoints=1)
    ...
    # ensure uniform binning regardless of range in data
    zbins = np.linspace(zlo, zhi, Nzbins + 1)
    plot_types(p3types, 'z', alldata, plotlist=p3list, xlabel='Redshift', ylabel='Number', plotdict=plotdict, bins=zbins,
               title=title)
    f.set_xlim(zlo, zhi)
    f.legend(loc='best', fontsize='small', numpoints=1)
    ...
    for sample, types, key, nc in zip(samplelist, typelist, dictkeys, ncolumns):
        nplot += 1
        f = fig.add_subplot(230 + nplot)
        plot_types(types, 'z', alldata, plotlist=[sample], yvar='mu', xlabel='Redshift', ylabel='$\mu$', plotdict=plotdict,
                   bins=zbins, title=plotlabels[key][sample], weights=False)
    ...
    if (args.xplots):  # check for extra plots
        ...
        f = fig.add_subplot(231)
        plot_types(p4types, 't0_err', alldata, plotlist=p4list, xlabel='$\Delta t_0$', ylabel='Number', plotdict=plotdict,
                   bins=t0ebins, title=title)
        f.legend(loc='best', fontsize='small', numpoints=1)

        f = fig.add_subplot(232)
        plot_types(p4types, 'x1_err', alldata, plotlist=p4list, xlabel='$\Delta x_1$', ylabel='Number', plotdict=plotdict,
            ...
            for l in MLdatalist:  # make plots for Test + any other "data"; will overflow to page 5a,b if needed
                ...
                for yvar, dy, lbl, ymn, ymx in zip(yvars, binwidy, ylabels, ymin, ymax):
                    ...
                    if (l in lbldMLdatalist):
                        ...
                        p5types = CLFtypes
                    plot_types(p5types, 't0_err', alldata, plotlist=[l], yvar=yvar, xlabel='SALT $\Delta t_0$', ylabel=lbl,
                               plotdict=plotdict, ctrxbin=t0_errbinwidth, ctrybin=dy, title=plotlabels[Data][l],
                               weights=False)
        ...
        for combos in [(0, 1), (0, 2), (1, 2)]:
            ...
            plotdict['labels'][contour] = 'SNIa Density'
            plot_types(rMLtypes, xvar, alldata, plotlist=[Training], yvar=yvar, xlabel=xlabel, ylabel=ylabel, cuts=pltcuts,
                       plotdict=plotdict, ctrxbin=dx, ctrybin=dy, title=plotlabels[Sim][Training], weights=False)
        ...
        for col, lo, hi, label in zip(SALTcolordiffs, binlo, binhi, SALTcolordifflabels):
            ...
            plot_types(plottypes, 'z', alldata, plotlist=plotlist, xlabel=xlabel, ylabel=ylabel, cuts=pltcuts,
                       plotdict=plotdict, plotid=SALT + col + diff + Fail, bins=zbins, alt=alt, addlabel='Fail ' + label)
            # f.set_xlim(-5.0, 5.0)
            axes = plt.gca()
            ...
            for filt, lo, hi, label, npl in zip(filts, binlo, binhi, labels, nplots):
                ...
                if (filt.find('ipeak') != -1 or filt.find('zpeak') != -1):
                    if (filt.find('ipeak') != -1):
                        ...
                        nplot = 6
                    f = fig.add_subplot(230 + nplot)
                    plot_types(plottypes, 'z', alldata, plotlist=plotlist, xlabel='Redshift', ylabel=ylabel, cuts=pltcuts,
    ...
    info_dict = {}
    info_dict['Purity'] = Pur
    info_dict['Efficiency'] = str(args.eff)
    info_dict['Features'] = str(args.ft)
    info_dict['Test Score'] = str(float(score))
    #print(str(float(P_eff_ref)))
    info_dict["Threshold Probability"] = str(round(float(P_eff_ref), 3))
    return info_dict
