...
def build_RF_classifier(data_train, nclass, features, ncores, alltypes_colname, target_class=0,
                        type_colnames={}, dummy=g.nodata,
                        n_estimators = 100, max_features = 'auto', min_samples_split = 5, criterion = 'entropy'):

    if len(type_colnames) == 0:
        print('  Column-name for appropriately labeled data needed but not specified')
        sys.exit()
...
def run_cross_validation(data, features, nclass, type_colnames={}):
...
def main(args):
    ...
    if not args.restore and train_format != args.train_format or args.restore and train_format != args.pklformat:
        print('\nWarning: resetting supplied training/pkl file-format argument to "{}" based on format of training/pkl data ({})'.format(train_format,
                                                                                                        data_files[g.Training]))      
    ...
    for dkey  in simlist + datalist:
        ...
        if g.Training not in dkey:           #read in data file and apply cuts
            ...
            data_this = read_data_file(dkey, data_files[dkey], SNRcut, zhi=g.zhi[zkey],
                                       zkey=g.feature_names['z'][format_this], format=file_formats[k],
