...
def parse_args(argv):
    ...
    parser.add_argument('--logfile', default=True, action='store_false',
                        help='Save printed output to logfile (see filedir/*_filestr.log)')
...
def get_features(features, data):
    ...
    for f in features:
        if not f in data.columns:
            g.printw('    Warning: column {} not found; replacing with dummy values {}'.format(f, g.nodata))
            data[f] = g.nodata
        flist.append(data[f])
        ...
        for f in features:
            if not f in data.columns:
                ...
                if len(alt_features) > 0:
                    ...
                    g.printw('    Replacing feature name {} with {} to match {} format'.format(f, alt_features[0], format))
...
def get_purity_scores(yset, probs, effcies, pos_label=0):
    ...
    g.printw('    Length of threshold array (unique prob. values) = {}'.format(len(thresh)))
    ...
    if len(x) > 1 and len(y) > 1:
        ...
        for effcy in effcies:
            ...
            g.printw('    Purity (P_thresh=0) = {:.3f}'.format(pur[pos_label]))
            score = score_func(probs, yset, effcy)
            g.printw('    SCORE (purity @ eff={:0.1f}%) = {:.4f}'.format(effcy * 100, score))
            P_eff_list.append(P_eff)
            score_list.append(score)
    else:
        g.printw('    Unable to interpolate efficiency function (insufficient unique values)')
        
    return pur, eff, thresh, P_eff_list, score_list


def get_ROC(yset_ROC, probs, pos_label=0, force_print=True):
    ...
    if len(np.unique(yset_ROC)) > 1:
        ...
        g.printw('\n  AUC = {:.3f}'.format(AUC), force_print=force_print)
    else:
...
def get_prob(clf, traindata, testdata, ytrain, debug=False, ntest=None):
    ...
    for nt, nodes in enumerate(allnodes[0:ntest]):
        test = testdata[nt]
        if (debug):
            g.printw("Validation data # {} = {}".format(nt, test))
            g.printw("Leaf nodes = {}".format(nodes))
        ...
        for n, node in enumerate(nodes):  # loop thru nodes in trees
            ...
            if (debug):
                g.printw('# of matching classes = {} for {}th tree'.format(len(classes), n))
                cldist = [np.sum(classes == x) for x in classvals]
                g.printw("# of each class = {}".format(cldist))
                g.printw("Prob from estimator = {}".format(clf.estimators_[n].predict_proba(X_test)[nt]))
            ...
            if (debug):
                g.printw("Probs = {} from #matches = {} in {}th tree".format(probs, len(classes), n))
...
def prob_errors(model, X, percentile=90, debug=False, ntest=None):
    ...
    for n, dtree in enumerate(model.estimators_):
        ...
        if (debug):
            g.printw("Probs = {} for {}th tree".format(probs[0:ntest], n))
        sumpr = sumpr + probs
...
def read_data_file(key, datafile, SNRcut=5, zhi=g.zhi, SNRkey='snr1', zkey='z',
                   format=g.default_format, doBazincuts=False, force_print=True):

    data = Table()
    status = False
    # check for acceptable data format
    if not any([f in datafile for v in g.allowed_formats.values() for f in v]):
        g.printw('  Unknown datafile format; expected formats are {}'.format(
    ...
    try:
        ...
        g.printw('{}'.format(msg), force_print=force_print)
        g.printw('{}'.format(''.join(['-' for c in msg])))
        data = read(datafile)
        g.printw('  Initial size: {}'.format(len(data)), force_print=force_print)
        ...
        if SNRkey in data.colnames:
            ...
            if nfail > 0:
                g.printw('  Removing {} entries failing {} > {}'.format(nfail, SNRkey, SNRcut))
                data = data[snrpass]
        else:
            g.printw('  Data column {} not found'.format(SNRkey))
        ...
        if zkey in data.colnames:
            ...
            if nzfail > 0:
                g.printw('  Removing {} entries failing {} > {}'.format(nzfail, zkey, zhi))
                data = data[zpass]
        else:
            g.printw('  Data column {} not found'.format(zkey))
        ...
        g.printw('  Total Size of {} Data (after cuts) = {}'.format(key, len(data)), force_print=force_print)
        g.printw('  Available feature names: \n  {}'.format(', '.join(data.colnames)))
        status = True

    except BaseException as ex:
        g.printw('  Error reading data file {}'.format(datafile), force_print=force_print)
        g.printw('  {}'.format(str(ex)), force_print=force_print)

    return data, status


def create_type_columns(data, nclass, all_class_values, alltypes_colname='type3',
                        desired_class_values=g.desired_class_values, force_print=True,
                        type_colnames={}, abort=False, format=g.default_format):

    status = True
    g.printw('\n  Checking format and values for columns with labeled data', force_print=force_print)
    ...
    if alltypes_colname not in data.colnames:
        g.printw('    Warning: requested alltypes-column-name {} not found'.format(alltypes_colname))
    else:
        ...
        if data[alltypes_colname].dtype != int:
            try:
                ...
                g.printw('    Found class-label values of {}'.format(' '.join(sntypes)))
                g.printw('    Converting string class labels to integers')
                ...
                g.printw('    Unable to convert type-labels {} to standard format: skipping use of class labels'.\
    ...
    try:
        ...
        if alltypes_colname != alltypes_colname_default and alltypes_colname_default not in data.colnames:
            ...
            g.printw('    Copying column {} to column {} (to be used for ROC curves etc)'.format(alltypes_colname,
        ...
        if str(nclass) in type_colnames.keys():
            ...
            for cname in colnames:
                ...
                if cname not in data.colnames or cname == 'type2x2': #force 2x2 recompute
                    g.printw('    Creating column "{}" used for {}-way typing'.format(cname, nclass), force_print=force_print)
                    if nclass == 2:  #binary class
                        g.printw('    Using {} to create "{}"'.format(alltypes_colname, cname))
                        ...
                        g.printw('    Attempting to use template information from data column {} to reconstruct SN types'.\
                                 format(g.generic_feature_names['sim_template'][format]))
                        data[cname] = g.desired_class_values[g.II]  # start with type 2
                        g.printw('    Initialized column "{}" with values = {}'.format(cname, g.desired_class_values[g.II]))
                        ...
                        g.printw('  Unknown column name {}'.format(colname))
                        status=False
...
def get_mask_from_templates(templates, sntypes=[g.Ia], Templates_Ia=[0]):
    ...
    g.printw('      Created mask for {} type {}s in data'.format(np.count_nonzero(mask), '+'.join(sntypes)))
...
def print_heading(v):
    ...
    g.printw('******************************************', force_print=True)
    g.printw('*              {}_v{}                *'.format(g.name, v), force_print=True)
    g.printw('*  {}  *'.format(g.description), force_print=True)
    g.printw('* {} *'.format(', '.join(g.authors)), force_print=True)
    g.printw('******************************************', force_print=True)    
...
def build_RF_classifier(data_train, nclass, features, ncores, target_class=g.target_class, ranseed=g.ranseed,
    ...
    g.printw('\n*************************', force_print=force_print)
    g.printw('*  BUILDING CLASSIFIER  *', force_print=force_print)
    ...
    if nclass == -2:  #staged classifiers in 2 steps
        ...
        g.printw("  Training set size = {};   Number of Ia's = {}".format(len(class_values), 
...
def get_ML_results(dkey, dataset, classifiers, MLtypes, class_values_list, effcy, nclass,
    ...
    if g.Ia not in list(all_class_values.keys()):         # require default SN typing
        g.printw('  Other class options/values not implemented yet')
    ...
    g.printw("\n{} DATA RESULTS\n  Target type: {}  (Class_value = {})".format(dkey.upper(),
             target_type, target_class), force_print=force_print)
    ...
    for n, (cl, true_classes) in enumerate(zip(classifiers, class_values_list)):
        ...
        g.printw('  Statistics for {} Data Set'.format(dkey))
        g.printw('  Total size = {}'.format(len(dataset[allprobs_colname])))
        ...
        g.printw('\n  Predictions using Fixed-Efficiency Classification:', force_print=force_print)
        for j, eff in enumerate(effcy):
            ...
            if target_type in list(all_class_values.keys()):  
                ...
                g.printw('Generic classification TBD')
            ...
            if n == len(classifiers) - 1:    #assemble final statistics
                ...
                get_prediction_summary(CLFid, MLtypes, masks[predict][dkey], dkey, target_type, true_classes,
                                       heading, force_print=force_print)
        ...
        if n == len(classifiers) - 1:    #assemble final statistics
            ...
            get_prediction_summary(CLFid, MLtypes, masks[g.MaxProb][dkey], dkey, target_type, true_classes, 
                                   heading, force_print=force_print)
            ...
            if true_classes is not None:
                ...
                g.printw('\n    Performance for {} data using Max. Prob. Classification:'.format(dkey), force_print=force_print)
                g.printw('      Purity = {:.3f}, Efficiency = {:.3f}'.format(Pur, Eff), force_print=force_print)
                g.printw('      Number of SN rejected = {} (Fraction = {:0.2f})'.format(Rem,
                             float(Rem)/len(dataset[allprobs_colname])), force_print=force_print)

    g.printw('\n  New columns added to data table:')
    g.printw('    {}'.format(', '.join([c for c in dataset.columns if c not in oldcols])))
...
def get_prediction_summary(CLFid, MLtypes, masks, dkey, target_type, true_classes, heading,
                           force_print=True):
    g.printw('\n    Predictions using {}:'.format(heading), force_print=force_print)
    for t in MLtypes:
        g.printw('\n      Predicted number of {}-Data {} = {}'.format(dkey, t,
                  np.count_nonzero(masks[CLFid + t])), force_print=force_print)
        if true_classes is not None:
            true_id = g.TP if t == target_type else g.TN
            false_id = g.FP if t == target_type else g.FN
            g.printw('        Correct (true) number of {}-Data {} = {}'.format(dkey, 
                      t, np.count_nonzero(masks[CLFid + t + true_id])), force_print=force_print)
            g.printw('        Incorrect (false) number of {}-Data {} = {}'.format(dkey, 
                      t, np.count_nonzero(masks[CLFid + t + false_id])), force_print=force_print)
...
def get_template_statistics(template_data, masks, MLtypes, classifications, dkey=g.Test, CLFid=g.RF,
    ...
    template_info = {}
    g.printw("\n  Template Statistics for {} Data".format(dkey), force_print=force_print)

    for cl in classifications:
        if cl == g.MaxProb:
            g.printw('\n    Maximum Probability Classification', force_print=force_print)
            cl_id = g.MaxProb
        else:
            g.printw('\n    Efficiency = {}'.format(cl), force_print=force_print)
            cl_id = g.Eff_.format(float(cl))
        # check for available classifications
        if CLFid + g.Ia not in masks[cl_id][dkey].keys():
            g.printw('    {} classification not available; skipping template statistics'.format(cl_id))
            continue
        template_info[cl_id] = {}
        ...
        g.printw('\n Templates for SN Classified as {}'.format(g.RFIa), force_print=force_print)
        g.printw('    Template | Value')
            ...
            for key in keys:
                if Iatemplate_dict[key] == freq:
                    if not (key in ordered_keys):
                        g.printw('    {:6} |{:6}'.format(key, freq))
                        ordered_keys.append(key)

        npop = 5
        g.printw('   {} most popular templates and frequencies for passing {}Ia {} {}'.format(npop, CLFid, 
                                                                                    ordered_keys[0:npop], 
...
def run_cross_validation(data, features, nclass, start_time=-1, nc=4, niter=5,
    ...
    if nclass == -2:
        ...
        for k in range(2, 11):
            ...
            g.printw('k={} folds CV scores : '.format(k), cv_scores_kf)
            kvals.append(k)
        ...
        for ts in np.arange(0.1, 1, test_step):
            g.printw('Fractional Validation Size : {}'.format(ts))
            ...
            for train_index, test_index in ss:
                ...
                g.printw("TRAIN SNIa: {} \tTEST SNIa: {}".format(np.count_nonzero(train1as),
...
def get_pscores_with_purities(classifiers, features, pfilename, purities, nclass, Fix_eff, SNRcut=None):
    ...
    for purity in purities:
        ...
        if SNRcut is not None:
            ...
            blurb = '(after SNR cut)'
        g.printw('Size of purity = {} test data {}: {}'.format(purity, blurb, len(ptest)))
        ...
        g.printw('Score for purity {} = {:0.4f}'.format(purity, pscore))
...
def get_HR(data, key, H0=68.62, Om0=.301, zkey='z', mukey='mu'):
    ...
    if mukey not in data.colnames:
        g.printw('  {} not available; filling Hubble Residual column for {} data with {}'.format(mukey,
                                                                                              key, g.nodata))
        data[g.HR] = g.nodata
    else:
        if 'sim_mu' not in data.colnames:
            ...
            try:
                ...
                g.printw('  Unable to compute Hubble residuals using columns true_mu/{}'.format(zkey))
        else:
...
def get_weights_old(masks, MLtypes, simlist, datalist, method=g.Constant, CLFid='RF', default=1.0, subclass=g.MaxProb):    
    ...
    for ckey in masks.keys():  # compute normalization factors
        ...
        for dkey, dmask in masks[ckey].items():
            ...
            g.printw('    Computing {} weights for {} Data:'.format(ntotal, dkey))
            ...
            for t, m in dmask.items():
                if data0 is not None:
                    if masks[ckey].has_key(data0) and masks[ckey][data0].has_key(t):   # t exists in data0
                        ...
                        if masks[subclass][data0].has_key(d0type):
                            g.printw('    Type {} not available for {} Data; using {} type {} instead'.format(t, data0, subclass, d0type))
                        else:
                            g.printw('    No appropriate substitute weights for {} in {} and {}'.format(t, dkey, data0))
                            d0type = None
                    if (method == g.ByType):  # accumulate weights by type
                        if np.count_nonzero(m) > 0 and np.count_nonzero(masks[ckey][data0][d0type]) > 0:
                            ...
                            g.printw('      Zero entries for type {} in {}; defaulting to weight derived from total numbers'.format(t, data0))
                ...
                if method == g.ByType:
                    g.printw('      w({}) = {:0.3f} {})'.format(t, norm[t], lengths))
            ...
            g.printw('      w{} = {:0.3f} {})'.format(blurb, totnorm, lengths))
...
def get_weights(alldata, use_data='', default=1.0):    
    ...
    for dkey in alldata.keys():
        ...
        if data0 is not None:
            ...
            weights = default
            lengths = ''
        g.printw('  w[{}] = {:0.3f} {}'.format(dkey, weights, lengths))
            ...
            if file_formats[k] in g.allowed_formats:
                    ...
                    if k in g.simulated_keys:
                        if alltypes_colnames[k] != g.data_defaults[file_formats[k]]['alltypes_colname_default']:
                            g.printw('  Warning: supplied alltypes_colname ({}) for {} data != default ({}) for file format {}'.\
                        ...
                        g.printw('  Including user-supplied data from {} with key/label {}; SN types: {}'.format(data_files[k], k, ulabel))
                #else:
                #    g.printw('  Skipping {} data; file {} not in training format ({})'.format(k, data_files[k], train_format))
            else:
                g.printw('  Skipping {} data; file {} not in allowed format'.format(k, data_files[k]))
                
    g.printw('\nProcessing {} data sets'.format(', '.join(simlist + datalist)), force_print=force_print)
    ...
    for k, f in g.alternate_feature_names[train_format].items():
        ...
        for v in fnames:
            if k in features and v in data.columns and k not in data.columns:
                ...
                data[k] = data[v]
                g.printw('  Copying column {} to {}'.format(v, k))
...
def check_alltypes_colname(dkey, alltypes_colname, file_format, data_file):
    ...
    if len(alltypes_colname) > 0:
        ...
        if alltypes_colname != g.data_defaults[file_format]['alltypes_colname'] and \
            ...
            g.printw('    Warning: resetting supplied alltypes_colname for {} data to "{}" (default for data file {})'.\
                      format(dkey, alltypes_colname, data_file))
        else:
            g.printw('    Column name {} OK: compatible with {} format'.format(alltypes_colname, file_format))
...
def check_all_class_values(all_class_values, file_format):
    ...
    if not all([v in g.data_defaults[file_format]['type_values'] for _,v in all_class_values.items()]):
        ...
        g.printw('    Warning: over-riding supplied arguments to {} based on {} data format'.format(all_class_values,
                                                                                                 file_format))
    else:
        g.printw('    Class values {} OK: compatible with {} format'.format(all_class_values,  file_format))
...
def exit_code(filename, status=g.SUCCESS, msg='', start_time=-1, force_print=True):
    if len(str(msg)) > 0:
        g.printw('\nException occurred: {}'.format(str(msg)), force_print=force_print)
    if start_time > 0:
        msg = "\nEnd-to-end runtime = {0:.3f} minutes\n"
        g.printw(msg.format((time() - start_time)/60), force_print=force_print)
    if len(filename) > 0 and filename != "''":
        with open(filename, 'w') as fh:
            fh.write(' {}'.format(g.SUCCESS if status==g.SUCCESS else g.FAILURE))
            g.printw('Wrote {}'.format(filename))
    msg = 'Completed with status: {}'.format(status)
    g.printw(msg, force_print=True)

    if g.logfile_handle is not None:
        g.logfile_handle.close()     #close logfile
...
def main(args, start_time=-1):
    ...
    # setup unique file-id for saving classifications, logfile, etc
    file_id = '{}'.format(args.filestr) if args.filestr != "''" else ''
    nway = '2x2' if args.nclass == -2 else args.nclass
    withhold_id = '_'.join(['withhold', '_'.join(args.withhold)]) if len(args.withhold) > 0 else ''
    ml_id = '_'.join([str(len(args.ft)) + 'features', str(nway) + 'way-typing', withhold_id])
    file_id = re.sub('_+', '_', '_'.join([ml_id, file_id, 'v'+version]))

    # setup logging and/or printing
    ...
    g.printw('\n{}-way classification'.format(str(nway)), force_print=True)
    
    # setup pklfilename
    pkldir, pklname = os.path.split(args.pklfile)
    pkldir = pkldir if len(pkldir) > 0 else args.filedir
    pkl_default = '{}way.pkl'.format('_'.join([args.pklformat, 'format', str(nway)]))
    ...
    g.printw('\nRunning checks on supplied/default arguments', force_print=True)
    ...
    if train_format not in g.allowed_formats:
        g.printw('  Training/pkl datafile ({}) may not be not in standard format: {} required'.format(data_files[g.Training], 
        ...
        if not args.use_filenames:
            ...
            g.printw('  Proceeding anyway: use_filenames was requested (user specified format assumed)')
            g.printw('  Assuming training/plk format {}'.format(supplied_format))
            train_format = supplied_format_key
            file_formats[g.Training] = train_format #save

    if  supplied_format_key != file_formats[g.Training]:
        g.printw('  Warning: over-riding supplied training/pkl file-format argument to "{}" (format of data file {})'.\
    ...
    if str(args.nclass) not in type_colnames:
        g.printw('  Data for {}-way typing not available for {} training-data format'.format(args.nclass, train_format))
        exit_code(args.done_file, status=g.FAILURE, start_time=start_time)
    ...
    if not args.restore:
        ...
        data_train, status = read_data_file(g.Training, data_files[g.Training], SNRcut=SNRcut, zhi=g.zhi[g.Training], 
                                            ...
                                            SNRkey=g.generic_feature_names['snrmx'][train_format], force_print=True,
        ...
        if len(args.withhold) > 0 and not args.restore:
            ...
            for withhold in args.withhold:
                if len(withhold) == 2: # 2-digit argument is a type
                    ...
                    data_train = data_train[~withcut]
                    withinfo='template'
                g.printw('Withholding {} {} from training sample'.format(withinfo, withhold), force_print=True)
    ...
    if args.restore:
        ...
        if args.nclass == -2:
            try:
                ...
                g.printw('\nUnable to load pre-classifier from pklfile {}'.format(pklfile), force_print=True)
                exit_code(args.done_file, status=g.FAILURE, start_time=start_time)
            g.printw('\nLoading pre-classifier from file pre{}'.format(pklfile), force_print=True)
            classifiers.insert(0, clfpre)
        g.printw('\nLoading classifier from pklfile {}'.format(pklfile), force_print=True)
        ...
        if CLFid == g.RF:
            ...
            if args.store:
                ...
                if args.nclass == -2:
                    ...
                    g.printw('\nSaving pre-classifier to file pre{}'.format(pklfile), force_print=True)
                joblib.dump(classifiers[-1], pklfile)
                g.printw('\nSaving classifier to file {}'.format(pklfile), force_print=True)
        else:
    ...
    # -------------------------------------------
    g.printw('\n************************', force_print=True)
    g.printw('*  RUNNING CLASSIFIER  *', force_print=True)
    g.printw('************************', force_print=True)
    ...
    g.printw('\nChecking requested data sets for allowed formats and compatibility', force_print=True)
    ...
    for dkey  in simlist + datalist:
        ...
        if g.Training not in dkey:           #read in data file and apply cuts
            ...
            if format_this not in g.allowed_formats:
                g.printw('  Skipping data file {} in unsupported format'.format(data_files[dkey]))
                continue
            ...
            if not status:
                g.printw('  Skipping data file {}'.format(data_files[dkey])) 
        else:
            data_this = data_train
            format_this = train_format

        if len(data_this) == 0:
            g.printw('  Skipping empty {} data file'.format(dkey))
            continue
        ...
        if len(alltypes_colname) > 0:
            ROC = {}
            g.printw('\n Evaluating ROC curves', force_print=True)
            ...
            fpr, tpr, roc_thres, AUC = get_ROC(y_test_ROC, data_this[g.RFprobability+str(target_class)], force_print=True)
            ...
            g.printw('    {}'.format(' '.join(['{:.3f}'.format(f) for f in fraction_true])))
        ...
        g.printw('\n  Saving classified data to {}'.format(filename), force_print=True)
        g.printw('    Saved columns:\n    {}'.format(', '.join(save_data.colnames)))
    ...
    g.printw('\nSaving performance to file {}'.format(pffile), force_print=True)
    ...
    if len(args.plots)==0 or args.plots[0] == '':
        g.printw("Skipping plots and exiting", force_print=True)
